...
class PeepSo
{
    ...
    private function __construct()
    {
        ...
        if(PeepSo::get_option_new('legacy_peepso_widget_position')) {
            add_action( 'widgets_init', function() {
                register_sidebar(array(
                    'name' => 'PeepSo Profile (Legacy)',
                    'id' => 'peepso',
                    'description' => 'Area reserved for PeepSo Integrated widgets. This feature will be phased out by the end of 2021.',9999
                ));
            },9999);
        ...
        if (is_admin() && !PeepSo::is_api_request()) {
            ...
            add_action('admin_notices', function() {
                ...
                if (!$current_month || isset($_GET['peepso_old_version_date'])) {
                    $months = range(3, 1200, 3); // generates: 3, 6, 9, 12, 15, ... 1200
        ...
        add_filter('wp_authenticate_user', function($user, $password) {
            if (isset($_POST['log'])) {
                ...
                $error = new WP_Error();
                if ($login_with_email == 2 && !is_email($_POST['log'])) {
                    $error->add('invalid_login', __('Invalid email address.', 'peepso-core'));
                    return $error;
                }

                if ($login_with_email == 1 && !is_email($_POST['log']) && isset($user->ID) && PeepSo::is_admin($user->ID)) {
                    $error->add('invalid_login', __('Invalid email address.', 'peepso-core'));
                    return $error;
                }
            }
...
//				$post = get_post($repost)
    ...
    public function login_recaptcha_validation($user, $username, $password)
    {
        ...
        if (class_exists('TwistPress_Social_Login') || class_exists('PeepSo_Social_Login') || class_exists('Social_Login')) {
